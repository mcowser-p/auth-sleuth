  ...
  plugins: [
    ...
    [
      ...
      "@semantic-release/exec",
      {
        ...
        publishCmd:
          'if [ -n "$GALAXY_API_KEY" ]; then ansible-galaxy collection publish mcowser_p-auth_sleuth-${nextRelease.version}.tar.gz --token "$GALAXY_API_KEY"; else echo "::warning::GALAXY_API_KEY not set — GitHub release only, Galaxy publish skipped"; fi',
      },
    ],
    [
      "@semantic-release/github",
      {
        ...
        assets: [
          { path: "mcowser_p-auth_sleuth-*.tar.gz", label: "Ansible collection (mcowser_p.auth_sleuth)" },
        ],
      },
    ],
  ],
